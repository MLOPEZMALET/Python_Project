			...
			while(i<len(listeElmPhrase)):
				...
				while(j<len(listeElmPhrase)):
					if(listeElmPhrase[j][0]==indiceAbs):
						indiceRel=str(j-i)
					j+=1
				fichierDepRel.write(listeElmPhrase[i][0]+'\t'+listeElmPhrase[i][1]+'\t'+listeElmPhrase[i][2]+'\t'+listeElmPhrase[i][3]+'\t'+listeElmPhrase[i][4]+'\t'+indiceRel+'\t'+listeElmPhrase[i][5]+'\t'+listeElmPhrase[i][6]+'\t'+listeElmPhrase[i][7]+'\n')
				i+=1
			fichierDepRel.write('\n')
			listeElmPhrase=[]
		cpt+=1
